...
EOF
  ...
  helm upgrade --wait -n "${grafana_namespace}" --install "${grafana_release_name}" \
    ...
    --set service.loadBalancerIP=10.220.82.10 \
...
scdf_namespace="scdf"
scdf_release_name="bsi-scdf-app"
grafana_ip=$(kubectl -n "${grafana_namespace}"  get svc "${grafana_release_name}"-grafana  -o jsonpath={.spec.loadBalancerIP})
scdf_ip="10.220.82.8"
...
if ! helm status "${scdf_release_name}" >/dev/null; then
  ...
  helm upgrade --wait -n "${scdf_namespace}" --install "${scdf_release_name}" bitnami/spring-cloud-dataflow \
    ...
    --set externalRabbitmq.host=rabbitmq-cl-scdf-01 \
    --set rabbitmq.enabled=false \
    --set metrics.enabled=true \
    --set metrics.serviceMonitor.enabled=true \
    --set metrics.serviceMonitor.namespace="${prometheus_namespace}" \
    --set server.service.type=LoadBalancer  \
    --set server.service.loadBalancerIP=${scdf_ip} \
    --set server.configuration.metricsDashboard="http://${grafana_ip}:3000" >/dev/null
fi
...
helm upgrade --wait -n "${scdf_namespace}" --install "${scdf_release_name}" bitnami/spring-cloud-dataflow \
  ...
  --set mariadb.auth.root.password=$MARIADB_ROOT_PASSWORD \
  --set externalRabbitmq.enabled=true \
  --set externalRabbitmq.host=rabbitmq-cl-scdf-01 \
  --set rabbitmq.enabled=false \
  --set metrics.enabled=true \
